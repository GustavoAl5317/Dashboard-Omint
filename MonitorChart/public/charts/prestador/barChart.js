...
// Função para calcular as somas e criar o gráfico apenas para a Prestador
async function createChart() {
    ...
    async function updateChart() {
        ...
        const PrestadorData = data.filter(obj => obj.csqname === 'Prestador');

        let totalTocadas = 0;
        let totalEspera = 0;

        PrestadorData.forEach(obj => {
    ...
    const myChart = new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                title: {
                    display: true,
                    text: 'Comparação entre Tocadas e Espera (Totais) - Prestador'
                }
            }
        }
    });
