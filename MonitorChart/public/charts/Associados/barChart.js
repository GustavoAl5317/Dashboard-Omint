...
// Função para calcular as somas e criar o gráfico apenas para a Associados
async function createChart() {
    ...
    async function updateChart() {
        ...
        const AssociadosData = data.filter(obj => obj.csqname === 'Associados');

        let totalTocadas = 0;
        let totalEspera = 0;

        AssociadosData.forEach(obj => {
    ...
    const myChart = new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                title: {
                    display: true,
                    text: 'Comparação entre Tocadas e Espera (Totais) - Associados'
                }
            }
        }
    });
