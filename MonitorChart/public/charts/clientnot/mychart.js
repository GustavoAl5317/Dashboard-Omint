...
const updateChart = (data) => {
  ...
  const clientnot = data.find(entry => entry.csqname === 'Cliente_nao_Identificado');

  if (clientnot) {
    // Atualizar os valores do gráfico com os valores da empresa
    myChart.data.datasets[0].data = [clientnot.handled, clientnot.total_aband];
    myChart.update();
  } else {
...
const fetchData = () => {
  const endpoint = "http://10.175.15.27/query";
  fetch(endpoint)
    .then(res => res.json())
    .then(data => {
      updateChart(data);
