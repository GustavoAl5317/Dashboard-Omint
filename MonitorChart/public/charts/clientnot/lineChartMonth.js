document.addEventListener('DOMContentLoaded', function() {
    ...
    function fetchDataFromRoute() {
        fetch('http://10.175.15.27/querymonth')
            .then(response => {
                if (!response.ok) {
    ...
    async function updateChartWithData(jsonData) {
        ...
        const datasets = {};
        const csqnameToDisplay = "Cliente_nao_Identificado"; // Defina o csqname que você deseja exibir
    ...
    setInterval(fetchDataFromRoute, 2000); // Atualiza a cada 60 segundos
});
