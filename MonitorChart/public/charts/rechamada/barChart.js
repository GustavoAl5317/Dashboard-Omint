...
// Função para calcular as somas e criar o gráfico apenas para a Rechamada
async function createChart() {
    ...
    async function updateChart() {
        ...
        const RechamadaData = data.filter(obj => obj.csqname === 'Rechamada');

        let totalTocadas = 0;
        let totalEspera = 0;

        RechamadaData.forEach(obj => {
    ...
    const myChart = new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                title: {
                    display: true,
                    text: 'Comparação entre Tocadas e Espera (Totais) - Rechamada'
                }
            }
        }
    });
