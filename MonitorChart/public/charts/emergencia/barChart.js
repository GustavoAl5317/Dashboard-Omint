...
// Função para calcular as somas e criar o gráfico apenas para a Emergencia
async function createChart() {
    ...
    async function updateChart() {
        ...
        const EmergenciaData = data.filter(obj => obj.csqname === 'Emergencia');

        let totalTocadas = 0;
        let totalEspera = 0;

        EmergenciaData.forEach(obj => {
    ...
    async function updateCounter() {
        ...
        document.getElementById('totalAtendidasCounter').innerText = `Total de Atendidas: ${totalAtendida}`;
    }
    ...
    document.getElementById('totalAtendidasCounter').innerText = `Total de Atendidas: ${totalAtendidas}`;

    const ctx = document.getElementById('myChartMonth').getContext('2d');
    const myChart = new Chart(ctx, {
        type: 'bar',
        data: {
            labels: ['Tocadas (seg)', 'Espera (Seg)'],
            datasets: [{
                label: 'Total',
        ...
        options: {
            ...
            plugins: {
                title: {
                    display: true,
                    text: 'Comparação entre Tocadas e Espera (Totais) - Emergencia'
                }
            }
        }
    });
