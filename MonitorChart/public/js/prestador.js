...
function atualizarValor() {
    ...
    .then(data => {
        ...
        if (data && data.length > 0) {
            ...
            const prestador = data.find(objeto => objeto.csqname === "Prestador");

            // Verificar se o objeto "Empresa" foi encontrado
            if (prestador) {
                // Atualizar o valor na div com o valor "offered" do objeto "prestador"
                const offeredValue =prestador.offered;
                document.getElementById('classNameId').innerText = offeredValue;

                // Calcular o nível de serviço (SL) em percentagem
                const serviceLevel =prestador.servicelevel;
                ...
                const maxQueueTime = prestador.max_queuetime;
                if (maxQueueTime !== undefined) {
                    ...
                    console.error('Propriedade "max_queuetime" não encontrada ou indefinida no objeto "prestador".');
                }

                // TME - tempo médio de espera

                const totalTME = prestador.queuetime;
                const totalHandled2 = prestador.handled;
                ...
                if (averageServiceTimeTme !== undefined) {
                    ...
                    console.error('Propriedade "Holdtime" não encontrada ou indefinida no objeto "prestador".');
                }

                // total chamadas abandonadas acima de 5 segundos
                const totalAbandHigher5 = prestador.aband5; // Corrigido o nome da variável
                document.getElementById('classNameId-totalAband-higher5').innerText = totalAbandHigher5;

                // total chamadas atendidas
                const totalHandled = prestador.handled;
                document.getElementById('classNameId-handled').innerText = totalHandled;


                // maior tempo de abandono
                const maxAbandonTime = prestador.max_time_aband; // Corrigido o nome da variável
                ...
                // tempo de conversa
                const totalTalkTime = prestador.talktime;
