...
function atualizarValor() {
    ...
    .then(data => {
        ...
        if (data && data.length > 0) {
            ...
            const clientnot = data.find(objeto => objeto.csqname === "Cliente_nao_Identificado");

            // Verificar se o objeto "Empresa" foi encontrado
            if (clientnot) {
                // Atualizar o valor na div com o valor "offered" do objeto "clientnot"
                const offeredValue =clientnot.offered;
                document.getElementById('classNameId').innerText = offeredValue;

                // Calcular o nível de serviço (SL) em percentagem
                const serviceLevel =clientnot.servicelevel;
                ...
                const maxQueueTime = clientnot.max_queuetime;
                if (maxQueueTime !== undefined) {
                    ...
                    console.error('Propriedade "max_queuetime" não encontrada ou indefinida no objeto "clientnot".');
                }

                // TME - tempo médio de espera

                const totalTME = clientnot.queuetime;
                const totalHandled2 = clientnot.handled;
                ...
                if (averageServiceTimeTme !== undefined) {
                    ...
                    console.error('Propriedade "Holdtime" não encontrada ou indefinida no objeto "clientnotência".');
                }

                // total chamadas abandonadas acima de 5 segundos
                const totalAbandHigher5 = clientnot.aband5; // Corrigido o nome da variável
                document.getElementById('classNameId-totalAband-higher5').innerText = totalAbandHigher5;

                // total chamadas atendidas
                const totalHandled = clientnot.handled;
                document.getElementById('classNameId-handled').innerText = totalHandled;


                // maior tempo de abandono
                const maxAbandonTime = clientnot.max_time_aband; // Corrigido o nome da variável
                if (maxAbandonTime !== undefined) {
                    ...
                    console.error('Propriedade "max_time_aband" não encontrada ou indefinida no objeto "clientnotencia".');
                }

                // tempo de conversa
                const totalTalkTime = clientnot.talktime;
