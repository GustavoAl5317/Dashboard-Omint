...
function atualizarValor() {
    ...
    .then(data => {
        ...
        if (data && data.length > 0) {
            ...
            const rechamada = data.find(objeto => objeto.csqname === "Rechamada");

            // Verificar se o objeto "Empresa" foi encontrado
            if (rechamada) {
                // Atualizar o valor na div com o valor "offered" do objeto "rechamada"
                const offeredValue =rechamada.offered;
                document.getElementById('classNameId').innerText = offeredValue;

                // Calcular o nível de serviço (SL) em percentagem
                const serviceLevel =rechamada.servicelevel;
                ...
                const maxQueueTime = rechamada.max_queuetime;
                if (maxQueueTime !== undefined) {
                    ...
                    console.error('Propriedade "max_queuetime" não encontrada ou indefinida no objeto "rechamada".');
                }

                // TME - tempo médio de espera

                const totalTME = rechamada.queuetime;
                const totalHandled2 = rechamada.handled;
                ...
                if (averageServiceTimeTme !== undefined) {
                    ...
                    console.error('Propriedade "Holdtime" não encontrada ou indefinida no objeto "rechamada".');
                }

                // total chamadas abandonadas acima de 5 segundos
                const totalAbandHigher5 = rechamada.aband5; // Corrigido o nome da variável
                document.getElementById('classNameId-totalAband-higher5').innerText = totalAbandHigher5;

                // total chamadas atendidas
                const totalHandled = rechamada.handled;
                document.getElementById('classNameId-handled').innerText = totalHandled;


                // maior tempo de abandono
                const maxAbandonTime = rechamada.max_time_aband; // Corrigido o nome da variável
                ...
                // tempo de conversa
                const totalTalkTime = rechamada.talktime;
