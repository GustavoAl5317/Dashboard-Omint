...
function atualizarValor() {
    ...
    .then(data => {
        ...
        if (data && data.length > 0) {
            ...
            const associados = data.find(objeto => objeto.csqname === "Associados");

            // Verificar se o objeto "Empresa" foi encontrado
            if (associados) {
                // Atualizar o valor na div com o valor "offered" do objeto "associados"
                const offeredValue =associados.offered;
                document.getElementById('classNameId').innerText = offeredValue;

                // Calcular o nível de serviço (SL) em percentagem
                const serviceLevel =associados.servicelevel;
                ...
                const maxQueueTime = associados.max_queuetime;
                if (maxQueueTime !== undefined) {
                    ...
                    console.error('Propriedade "max_queuetime" não encontrada ou indefinida no objeto "associados".');
                }

                // TME - tempo médio de espera

                const totalTME = associados.queuetime;
                const totalHandled2 = associados.handled;
                ...
                if (averageServiceTimeTme !== undefined) {
                    ...
                    console.error('Propriedade "Holdtime" não encontrada ou indefinida no objeto "associadosência".');
                }

                // total chamadas abandonadas acima de 5 segundos
                const totalAbandHigher5 = associados.aband5; // Corrigido o nome da variável
                document.getElementById('classNameId-totalAband-higher5').innerText = totalAbandHigher5;

                // total chamadas atendidas
                const totalHandled = associados.handled;
                document.getElementById('classNameId-handled').innerText = totalHandled;


                // maior tempo de abandono
                const maxAbandonTime = associados.max_time_aband; // Corrigido o nome da variável
                if (maxAbandonTime !== undefined) {
                    ...
                    console.error('Propriedade "max_time_aband" não encontrada ou indefinida no objeto "associadosencia".');
                }

                // tempo de conversa
                const totalTalkTime = associados.talktime;
